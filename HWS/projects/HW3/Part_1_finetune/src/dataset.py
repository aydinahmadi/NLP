...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        trunc_len = int(random.randint(4, int(self.block_size * 7 / 8)))
        trunc_len = min(len_document, trunc_len)
        trunc_doc = document[:trunc_len]

        avg_mask = int(trunc_len / 4)

        after_mask = int(random.randint(1, max(2 * avg_mask, 1)))

        before_mask = int(random.randint(0, trunc_len - avg_mask + 1))
        prefix, masked_content, suffix = trunc_doc[:before_mask], trunc_doc[before_mask:before_mask + after_mask], \
                                         trunc_doc[before_mask + after_mask:]
