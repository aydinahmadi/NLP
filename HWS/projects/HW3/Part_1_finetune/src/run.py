...
writer = SummaryWriter(log_dir='expt/%s/%s_%d_pt_lr_%f_ft_lr_%f' % (
    args.function,
    args.tb_expt_name,
    args.bottleneck_dim,
    args.pretrain_lr,
...
if args.function == 'pretrain':
    ...
    mconf2 = trainer.TrainerConfig(max_epochs=650, batch_size=128, learning_rate=args.pretrain_lr, lr_decay=True,
                                   warmup_tokens=512 * 20, final_tokens=200 * len(pretrain_dataset) * block_size,
                                   num_workers=4, writer=writer)

    trainer = trainer.Trainer(model,pretrain_dataset, None, mconf2)
    trainer.train()
    ...
    if args.reading_params_path is not None:
        ...
        mconf3 = trainer.TrainerConfig(max_epochs=10, batch_size=256, learning_rate=args.finetune_lr,
                                       lr_decay=True, warmup_tokens=512 * 20,
                                       final_tokens=200 * len(pretrain_dataset) * block_size,
                                       num_workers=4, writer=writer)

    else:
        mconf3 = trainer.TrainerConfig(max_epochs=75, batch_size=256, learning_rate=args.finetune_lr,
                                       lr_decay=True, warmup_tokens=512 * 20,
                                       final_tokens=200 * len(pretrain_dataset) * block_size,
                                       num_workers=4, writer=writer)

    dataset_read = open(args.finetune_corpus_path).read()
    dataset_load = dataset.NameDataset(pretrain_dataset, dataset_read)
    trainer2 = trainer.Trainer(model, dataset_load, None, mconf3)
    trainer2.train()
    torch.save(model.state_dict(), args.writing_params_path)
